...
const Home: NextPage = ({
  ...
  const csv_report = {
    filename: "symptom.csv",
    headers: headers,
    data: symptom,
  };

  return (
    <Container title="Symptom">
      <div className="layout">
        <h2 className="mb-4">Symptom</h2>
        <div className="flex items-center justify-end w-full mb-8 space-x-3">
          <div className="text-xs text-gray-600">
            {symptom?.length ?? 0} record(s)
          </div>
          <CSVLink {...csv_report}>
...
export const getServerSideProps: GetServerSideProps = async () => {
  const symptom = await await prisma.symptom.findMany();

  return {
    props: { symptom: JSON.parse(JSON.stringify(symptom)) },
  };
};
