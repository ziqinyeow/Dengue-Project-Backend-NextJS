...
const DeleteAdminCard = ({ show, setShow, data }: any) => {
      ...
      {show !== 0 && (
        <div className="fixed top-0 left-0 z-50 flex items-center justify-center w-full h-full py-5 bg-gray-200 bg-opacity-50">
          <div className="relative w-full max-w-3xl max-h-full px-10 py-10 mx-auto overflow-y-auto text-black bg-white rounded-lg shadow-2xl no_scrollbar 2xl:max-w-5xl">
            <h3 className="mb-5 font-bold">Delete Admin</h3>
            <button
              disabled={uploading}
              type="button"
              onClick={() => {
            ...
            >
              <h5 className="mb-5 text-sm text-gray-500 select-none">
                Note that this action will permanently remove {data?.type} (
                <span className="text-blue-700">{data?.email}</span>)
              </h5>
              <h5 className="mb-5 text-sm text-gray-500 select-none">
                To delete this {data?.type}, please re-enter the email below.
              </h5>
              <input
                type="email"
                placeholder={data?.email}
                className="w-full px-4 py-2 mb-8 bg-white border rounded-md disabled:text-gray-400 placeholder:text-gray-200 disabled:bg-gray-200/50 focus:outline-none focus:border-gray-400"
                name="email"
                onChange={change}
                required
                autoComplete="off"
